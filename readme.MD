...
You should really be using the [built-in OBS Virtual Camera](https://obsproject.com/kb/virtual-camera-guide) over this plugin. This is used as a stop-gap for things that don't yet work with the built-in Virtual Camera (like ~~<https://github.com/obsproject/obs-studio/issues/3635>, <https://github.com/opencv/opencv/issues/19746>~~, and multi-camera support). I do not plan on supporting this fork outside of necessary updates if OBS breaks the plugin. And only plan on doing so until the aforementionned OpenCV issue is fixed.
